...
void SampleWaterHeight_float(UnityTexture2D terrainWaterHeightmap, float2 uv, float2 elevationRange, out float height)
{
    float2 terrainHeight = terrainWaterHeightmap.SampleLevel(terrainWaterHeightmap.samplerstate, uv, 0).xy;
    height = terrainHeight.x + terrainHeight.y; // x = terrain height, y = water height
    height = height * (elevationRange.y - elevationRange.x) + elevationRange.x;
}
